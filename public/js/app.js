    ...
    window.Alpine.data("ToastComponent", function ($wire) {
      return {
        ...
        fetchWireToasts: function fetchWireToasts() {
          ...
          this.wireToasts.forEach(function (toast) {
            _this2.add(window.Alpine.raw(toast));
          }), this.wireToasts.length > 0 && $wire.set("toasts", []);
        },
        add: function add(toast) {
          var _toast$type;

          !0 === this.loaded ? (null !== (_toast$type = toast.type) && void 0 !== _toast$type || (toast.type = "info"), toast.show = 0, toast.index = this.count, this.toasts[this.count] = toast, this.scheduleRemoval(this.count), this.count++) : this.pendingToasts.push(toast);
        },
